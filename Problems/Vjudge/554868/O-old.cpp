...
int out[N];
struct Operation { int type, v1, v2, id, coe; };
struct SegmentTree {
	...
	void solve(int x = 1, int l = 1, int r = K) {
		...
		for(auto &p : t[x])
			...
			else out[p.id] += p.coe * trie.query(p.v1, p.v2);
		if(l == r) return;
		int mid = (l + r) >> 1;
...
int main() {
	...
	for(int i = 1; i <= n; i++) {
		seg1.insert(bi[i], {0, a[i], b[i], 0, 0}), seg2.insert(bi[i], {0, a[i], 0, 0, 0});
		for(auto p : q[i]) {
			seg1.query(1, p.di, {1, p.c, 0, p.id, p.t});
			seg2.query(p.di + 1, K, {1, p.c, p.d, p.id, p.t});
		}
	}
