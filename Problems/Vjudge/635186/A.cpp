...
int main() {
	...
	invfac[D] = qpow(fac[D], MOD - 2);
	for(int i = D; i >= 1; i--) invfac[i - 1] = invfac[i] * i % MOD;
	f.resize(D + 1);
	for(int i = 0; i <= D; i++) f[i] = qpow(2, (-(LL)i * (i - 1) / 2) % (MOD - 1) + (MOD - 1)) * invfac[i] % MOD;
	f *= f;
	f.resize(D + 1);
	for(int i = 0; i <= D; i++) f[i] = f[i] * qpow(2, (LL)i * (i - 1) / 2 % (MOD - 1)) % MOD;
	// for(int i = 0; i <= D; i++) printf("f[%d] = %lld\n", i, f[i] * fac[i] % MOD);
	f = f.sqrt();
