...
struct Polynomial { LL w[N * 4]; int len; Polynomial() : len(0) { memset(w, 0, sizeof(w)); } };
int rev[N * 4];
...
Polynomial ln(Polynomial x) { return integrate(mul(derivate(x), inv(x), x.len)); }
