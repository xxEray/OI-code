...
int main() {
	...
	Point<double> center;
	for(int i = 0; i < n; i++) center = Point<double>(center.x + a[i].x, center.y + a[i].y);
	center = Point<double>(center.x / n, center.y / n);
	std::sort(a.begin(), a.end(), [&](const Point<double> &x, const Point<double> &y) { return polar_compare(Vector<double>(center, x), Vector<double>(center, y)); });
	double ans = 0;
	auto solve = [&]() {
		// for(auto [x, y] : a) printf("%.3f %.3f\n", x, y);
		// puts("");
		int sz = a.size();
		for(int i = 0; i < sz; i++) if(a[i] == a[(i + 1) % sz]) { a.erase(a.begin() + i); return; }
		auto bisector = [&](Point<double> x, Point<double> y, Point<double> z) { return Line<double>(x, Vector<double>(x, y).unit() + Vector<double>(x, z).unit()); };
		int opt = 0;
		double odis = 1e20;
		for(int i = 0; i < sz; i++) {
			auto p = intersect(bisector(a[i], a[(i + sz - 1) % sz], a[(i + 1) % sz]), bisector(a[(i + 1) % sz], a[i], a[(i + 2) % sz]));
			double dis = distance(p, Line<double>(a[i], a[(i + 1) % sz]));
			if(dis < odis) opt = i, odis = dis;
		}
		{
			auto p = intersect(bisector(a[opt], a[(opt + sz - 1) % sz], a[(opt + 1) % sz]), bisector(a[(opt + 1) % sz], a[opt], a[(opt + 2) % sz]));
			ans += edis(a[opt], p);
		}
		std::vector<Line<double>> vct(sz);
		for(int i = 0; i < sz; i++) {
			Line<double> line(a[i], a[(i + 1) % sz]);
			line.p = line.p + Vector<double>::from_point(-line.c.y, line.c.x).unit() * odis;
			vct[i] = line;
		}
		std::vector<Point<double>> b;
		for(int i = 0; i < sz; i++) if(i != opt) {
			auto p = intersect(vct[(i - 1 + sz) % sz], vct[i]);
			ans += edis(a[i], p);
			b.emplace_back(p);
		}
		a.swap(b);
	};
	while(a.size() > 2) solve();
	if(a.size() == 2) ans += edis(a[0], a[1]);
	printf("%.15Lf\n", ans);
	return 0;
} /*
4
