...
int main() {
	...
	while(T--) {
		...
		auto conv = Convex<LL>(vct);
		// for(auto [x, y] : conv.full_convex()) printf("(%lld, %lld) ", x, y);
		int cnt = 0;
		for(auto p : vct) cnt += p.on(conv);
		if(cnt < 4) {
			if(conv.size() < 3) { puts("0"); continue; }
			LL full = std::abs(cross(conv[0] - conv[1], conv[0] - conv[2])), mn = LLINF;
			for(auto p : vct) if(p != conv[0] && p != conv[1] && p != conv[2]) {
				mn = std::min(mn, std::abs(cross(conv[0] - p, conv[1] - p)));
				mn = std::min(mn, std::abs(cross(conv[1] - p, conv[2] - p)));
				mn = std::min(mn, std::abs(cross(conv[0] - p, conv[2] - p)));
			}
			full -= mn;
			if(full & 1) printf("%lld.5\n", full);
			else printf("%lld\n", full / 2);
			continue;
		}
		LL ans = 0;
		int sz = conv.size();
		auto solve = [&](int p, int q) {
			// printf("p=%d, q=%d\n", p, q);
			LL mx1 = 0, mx2 = 0;
			for(int i = p; i != q; i = (i + 1 == sz ? 0 : i + 1)) mx1 = std::max(mx1, std::abs(cross(conv[p] - conv[i], conv[q] - conv[i])));
			for(int i = q; i != p; i = (i + 1 == sz ? 0 : i + 1)) mx2 = std::max(mx2, std::abs(cross(conv[p] - conv[i], conv[q] - conv[i])));
			ans = std::max(ans, mx1 + mx2);
		};
		conv.rotating_calipers([&](int i, int j, int k) { solve(i, k), solve(j, k); });
