...
LL F(int x, int l, int r) {
	// C(2x + l, x) ~ C(2x + r, x)
	return (C(2 * x + r + 1, x + 1) - C(2 * x + l, x + 1) + MOD) % MOD;
}
...
int main() {
	...
	while(T--) {
		...
		if(cross(y - x, z - x) < 0) std::swap(y, z);
		// printf("(%lld, %lld) (%lld, %lld) (%lld, %lld)\n", x.x, x.y, y.x, y.y, z.x, z.y);
		LL ans = 0;
		for(int i = 0; i <= 1000000; i++) {
			Line<LL> now(Point<LL>(-i, i), Vector<LL>::from_point(1, 1));
			int l = -i, r = INF;
			bool flag = true;
			auto solve = [&](Line<LL> t) {
				if(parallel(now, t)) {
					if(same_direction(now, t)) {
						if(t.p.side(now) > 0) flag = false;
					} else {
						if(t.p.side(now) < 0) flag = false;
					}
				} else {
					if(cross(Vector<LL>::from_point(1, 1), t.c) > 0 && cross(t.c, Vector<LL>::from_point(-1, -1)) > 0) {
						r = std::min(r, (int)floor(intersect(t, now).x));
					} else {
						l = std::max(l, (int)ceil(intersect(t, now).x));
					}
				}
			};
			solve(Line<LL>(x, y)), solve(Line<LL>(y, z)), solve(Line<LL>(z, x));
			// printf("%d: l = %d, r = %d (%d)\n", i, l, r, flag);
			if(flag && l <= r) (ans += F(i, l, r)) %= MOD;
		}
		printf("%lld\n", ans);
	}
	return 0;
