...
int main() {
	scanf("%d", &n);
	for(int i = 1; i <= n; i++) { int x, y; scanf("%d%d", &x, &y); vct.emplace_back(x, y); }
	Convex<LL> conv(vct);
	// printf("conv: "); for(auto [x, y] : conv.full_convex()) printf("(%lld, %lld) ", x, y); puts("");
	int sz = conv.size();
	double ans = 1e20;
	int id0, id1, id2, id3, id4;
	auto makeperp = [&](Vector<LL> p) { return Vector<LL>::from_point(p.y, -p.x); };
	conv.rotating_calipers([&](int p, int q, int t) {
		// printf("(%d %d %d)\n", p, q, t);
		Line<LL> line(conv[p], conv[q]);
		double side1 = distance(conv[t], line);
		Line<LL> perp(conv[t], makeperp(line.c));
		double side2 = 0;
		int nid1 = -1, nid2 = -1;
		auto max_dis = [&](int l, int r) {
			if(l > r) r += sz;
			// printf("l = %d, r = %d\n", l, r);
			while(l < r) {
				int mid = (l + r) >> 1;
				if(std::abs(cross(perp.c, Vector<LL>(conv[t], conv[mid % sz]))) < std::abs(cross(perp.c, Vector<LL>(conv[t], conv[(mid + 1) % sz])))) l = mid + 1;
				else r = mid;
			}
			// printf("l = %d\n", l);
			(nid1 != -1 ? nid2 : nid1) = l % sz;
			side2 += distance(conv[l % sz], perp);
		};
		max_dis(t, p), max_dis(q, t);
		// printf("side1 = %.5f, side2 = %.5f\n", side1, side2);
		if(ans > side1 * side2) {
			// printf("update! id1=%d, id2=%d\n", nid1, nid2);
			ans = side1 * side2;
			id0 = t, id1 = nid1, id2 = nid2, id3 = p, id4 = q;
		}
	});
	// printf("%.5f\n", ans);
	// printf("(%d %d %d %d %d)\n", id0, id1, id2, id3, id4);
	Line<LL> line(conv[id3], conv[id4]);
	auto p1 = intersect(Line<LL>(conv[id0], line.c), Line<LL>(conv[id1], makeperp(line.c)));
	auto p2 = intersect(Line<LL>(conv[id1], makeperp(line.c)), line);
	auto p3 = intersect(line, Line<LL>(conv[id2], makeperp(line.c)));
	auto p4 = intersect(Line<LL>(conv[id2], makeperp(line.c)), Line<LL>(conv[id0], line.c));
	printf("%.6f %.6f\n", p1.x, p1.y);
	printf("%.6f %.6f\n", p2.x, p2.y);
	printf("%.6f %.6f\n", p3.x, p3.y);
	printf("%.6f %.6f\n", p4.x, p4.y);
