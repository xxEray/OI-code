...
template<typename T> struct Convex {
	...
	const Point<T> &operator[](int) const;
};
...
template<typename T> Point<T> &Convex<T>::operator[](int x) {
	if(x < (int)up.size()) return up.rbegin()[x];
	else return dn[x - (int)up.size() + (up.front() == dn.front())];
}
template<typename T> const Point<T> &Convex<T>::operator[](int x) const {
	if(x < (int)up.size()) return up.rbegin()[x];
	else return dn[x - (int)up.size() + (up.front() == dn.front())];
}
...
int n;
std::vector<Point<LL>> convex, other;
...
int main() {
	...
	for(int i = 1; i <= n; i++) {
		int x, y, t;
		scanf("%d%d%d", &x, &y, &t);
		if(t) convex.emplace_back(x, y);
		else other.emplace_back(x, y);
	}
	auto conv = Convex(convex);
	if(conv.size() == 1) {
		int cnt = convex.size();
		if(cnt == 1) puts("1");
		else {
			for(auto p : other) cnt += (p == convex.front());
			printf("%d\n", cnt);
		}
		return 0;
	}
	// printf("conv: "); for(int i = 0; i < conv.size(); i++) printf("(%lld, %lld) ", conv[i].x, conv[i].y); puts("");
	int base = convex.size();
	std::vector<std::pair<Vector<LL>, bool>> seq;
	for(auto p : other) {
		if(p.in(conv)) { base++; continue; }
		auto pp = p.tangent(conv);
		// printf("(%lld, %lld): ((%lld, %lld), (%lld, %lld))  ((%lld, %lld), (%lld, %lld))\n", p.x, p.y, pp.first.p.x, pp.first.p.y, (pp.first.p + pp.first.c).x, (pp.first.p + pp.first.c).y,
			// pp.second.p.x, pp.second.p.y, (pp.second.p + pp.second.c).x, (pp.second.p + pp.second.c).y);
		auto l = pp.first.c, r = pp.second.c;
		// printf("l = (%lld, %lld), r = (%lld, %lld)\n", l.x, l.y, r.x, r.y);
		auto is_upper = [&](const Vector<LL> &t) { return t.y > 0 || (t.y == 0 && t.x > 0); };
		if(is_upper(l) && is_upper(r)) seq.emplace_back(l, 0), seq.emplace_back(r, 1);
		else if(!is_upper(l) && !is_upper(r)) seq.emplace_back(-l, 0), seq.emplace_back(-r, 1);
		else if(is_upper(l) && !is_upper(r)) seq.emplace_back(l, 0), seq.emplace_back(Vector<LL>::from_point(1, 0), 0), seq.emplace_back(-r, 1);
		else seq.emplace_back(-l, 0), seq.emplace_back(Vector<LL>::from_point(1, 0), 0), seq.emplace_back(r, 1);
		// for(auto [t, v] : seq) printf("(%lld, %lld) %d\n", t.x, t.y, v);
	}
	std::sort(seq.begin(), seq.end(), [&](const std::pair<Vector<LL>, bool> &x, const std::pair<Vector<LL>, bool> &y) {
		return same_direction(x.first, y.first) ? x.second < y.second : cross(x.first, y.first) > 0;
	});
	// puts("-----");
	// for(auto [p, v] : seq) printf("(%lld, %lld) %d\n", p.x, p.y, v);
	// printf("base = %d\n", base);
	int mn = INF, tot = 0;
	if(seq.empty() || cross(seq.front().first, Vector<LL>::from_point(1, 0)) < 0) mn = 0;
	for(int i = 0; i < (int)seq.size(); i++) {
		tot += seq[i].second ? -1 : 1;
		if(i == (int)seq.size() - 1 || !same_direction(seq[i].first, seq[i + 1].first) || seq[i].second != seq[i + 1].second) mn = std::min(mn, tot);
	}
	printf("%d\n", base + mn);
	return 0;
}