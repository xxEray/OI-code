...
template<typename T> struct Point {
	...
	bool on(const Point<T> &, const Point<T> &) const;
	bool strictly_on(const Point<T> &, const Point<T> &) const;
...
template<typename T> double edis(const Point<T> &x, const Point<T> &y) { return std::sqrt((double)((x.x - y.x) * (x.x - y.x) + (x.y - y.y) * (x.y - y.y))); }
template<typename T> struct Vector {
	T x, y;
	Vector() : x(), y() {}
	...
	Vector<double> unit() const { auto len = length(); geometry_assert(gt(len, (double)0)); return Vector<double>::from_point((double)x / len, (double)y / len); }
private:
...
template<typename T> struct Line {
	...
	Line<double> unit() const { return Line<double>((Point<double>)p, c.unit()); }
...
template<typename T> struct Convex {
	...
	T diameter_sq() const;
	double diameter() const { return std::sqrt((double)diameter_sq()); }
...
template<typename T> bool Point<T>::on(const Point<T> &p, const Point<T> &q) const { return on(Line<T>(p, q)) && !gt(dot(Vector<T>(*this, p), Vector<T>(*this, q)), (T)0); }
template<typename T> bool Point<T>::strictly_on(const Point<T> &p, const Point<T> &q) const { return on(Line<T>(p, q)) && lt(dot(Vector<T>(*this, p), Vector<T>(*this, q)), (T)0); }
...
template<typename T> Point<T> intersect(const Line<T> &x, const Line<T> &y, T &d) {
	...
	d = s1 + s2;
	return Point<T>((Vector<T>)x.p * (s1 + s2) + x.c * s1);
}
...
int main() {
	scanf("%d", &n);
	for(int i = 0; i < n; i++) { int x, y; scanf("%d%d", &x, &y); a.emplace_back(x, y); }
	auto calc = [&](const Point<LL> &x, const Point<LL> &y) {
		// printf("x = (%lld, %lld), y = (%lld, %lld)\n", x.x, x.y, y.x, y.y);
		std::vector<std::pair<double, int>> vct;
		auto sign = [&](LL val) { return val > 0 ? 1 : (val == 0 ? 0 : -1); };
		for(int i = 0; i < n; i++) {
			auto p = a[i], q = a[(i + 1) % n];
			int w = sign(q.side(Line<LL>(x, y))) - sign(p.side(Line<LL>(x, y)));
			if(!w) continue;
			auto inter = intersect(Line<LL>(x, y), Line<LL>(p, q));
			vct.emplace_back(dot(inter - (Point<double>)x, (Vector<double>)(y - x)) / (y - x).length(), w);
		}
		std::sort(vct.begin(), vct.end());
// 		printf("vct: "); for(auto [d, v] : vct) printf("(%.3f, %d) ", d, v); puts("");
		double r = dot(y - x, y - x) / (y - x).length();
		double lb = -1e20, rb = -1e20;
		bool fl = true;
		int sum = 0;
		for(int i = 0; i < (int)vct.size(); i++) {
			sum += vct[i].second;
			if((i == (int)vct.size() - 1 || vct[i].first < vct[i + 1].first - eps)) {
				if(vct[i].first < r - eps) {
					if(lb <= -1e20 + eps) lb = (sum ? vct[i].first : -1e20);
					else { if(!sum) lb = -1e20; }
				}
				if(vct[i].first >= -eps && fl) {
					if(sum) rb = (i == (int)vct.size() - 1 ? 1e18 : vct[i + 1].first);
					else fl = false;
				}
			}
		}
// 		printf("lb = %.5f, rb = %.5f\n", lb, rb);
		if(lb <= -1e20 + eps || rb <= -1e20 + eps) return 0.;
		else if(lb > eps || rb < r - eps) return 0.;
		else return rb - lb;
	};
	double ans = 0;
	for(int i = 0; i < n; i++) for(int j = i + 1; j < n; j++) ans = std::max(ans, calc(a[i], a[j])); // , printf("! %d %d (ans = %.5f)\n", i, j, ans);
	printf("%.9f\n", ans);
	return 0;
} /*
4
