...
int main() {
	...
	while(T--) {
		LL tmpx, tmpy;
		scanf("%d%lld%lld%d", &n, &tmpx, &tmpy, &c);
		p = tmpx * inv(tmpy) % MOD, q = (MOD + 1 - p) % MOD;
		for(int i = 0; i < n; i++) a[i] = b[i] = 0;
		for(int i = 0; i <= c - 1; i++) a[i] = C(c - 1, i) * qpow(q, c - 1 - i) % MOD;
		for(int i = 0; i <= n - c; i++) b[i] = C(n - c, i);
		mul(a, b, a, n - 1);
		for(int i = 0; i < n; i++) (a[i] *= inv(MOD + 1 - qpow(q, n - i)) * fac[n - 1 - i] % MOD) %= MOD;
		for(int i = 0; i < n; i++) b[i] = invfac[i] * qpow(MOD - 1, i) % MOD;
		mul(a, b, a, n - 1);
		for(int i = 0; i < n; i++) (a[i] *= invfac[n - 1 - i]) %= MOD;
