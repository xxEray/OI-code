...
int main() {
	...
	while(T--) {
		...
		while(m--) {
			...
			if(t == 1) {
				scanf("%d%d", &i, &x);
				if(x == K) add_edge(hash(i, x), hash(i, x) + n * K);
				...
				for(int k = x + 1; k <= K; k++) add_edge(hash(i, k), hash(i, k) + n * K), add_edge(hash(j, k), hash(j, k) + n * K);
			} else if(t == 3) {
				scanf("%d%d%d", &i, &j, &x);
				...
				for(int k = 1; k <= K; k++) if(k - 1 + K < x) add_edge(hash(i, k) + n * K, hash(i, k)), add_edge(hash(j, k) + n * K, hash(j, k));
			}
		}
