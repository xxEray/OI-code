...
class Polynomial {
	...
	static inline const std::vector<int> &get_power_G(int len) {
	...
	static inline void DIF(std::vector<int> &arr) {
	...
	static inline void DIT(std::vector<int> &arr) {
	...
	static inline const std::vector<int> &get_invs(int l) {
...
public:
	...
	explicit operator std::vector<int>() const { return a; }
	constexpr inline int getMOD() const { return MOD; }
	constexpr inline int getG() const { return G; }
	...
	inline Polynomial inv(int len) const {
	...
	inline Polynomial sqrt_qr(int len, int qr) const {
	...
	inline Polynomial pow(int y) const {
		if(y == 0) {
	...
	inline Polynomial pow(int y_mod, int y_phi) const {
		int len = SZ(a);
	...
	friend inline Polynomial mulT(const Polynomial &x, Polynomial y) {
		std::reverse(y.a.begin(), y.a.end());
		int n = SZ(x.a), m = SZ(y.a);
		x *= y;
		return x.subpoly(m - 1, m + n - 1);
	}
	friend inline std::vector<int> multi_point(Polynomial f, std::vector<int> vct) {
		int sz_vct = SZ(vct);
		int n = std::max(SZ(f.a), SZ(vct));
		f.a.resize(n), vct.resize(n);
		...
		dfs(dfs, 1, 0, n - 1, mulT(f, Q[1].inv()));
		ret.resize(sz_vct);
		return ret;
	}
	static inline Polynomial interpolate(const std::vector<int> &X, const std::vector<int> &Y) {
...
using Poly = Polynomial<MOD, 3>;

int main() {
	read(n), read(K);
	std::vector<int> vct;
	for(int i = 0; i <= n; i++) { int x; read(x); vct.emplace_back(x); }
	Poly f(vct);
	f = derivative(pow(Poly(1) + ln(Poly(2) + f - Poly(f.evaluate(0)) - exp(integral(inv(sqrt_qr(f, quadratic_residue))))), K));
